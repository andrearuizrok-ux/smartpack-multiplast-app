...
(()=>{
  ...
  async function gmailCall(action,extra={}){
    ...
    const {data,error}=await sb.functions.invoke('poi-gmail-orders',{body:{action,...extra}});
  ...
  function gmailStateHtml(){
    ...
    if(gmailStatus.connected){
      ...
      return `<b>Gmail collegato · ${esc(c.google_email||'casella ordini')}</b><span>${c.last_sync_at?'Ultima sincronizzazione: '+esc(new Date(c.last_sync_at).toLocaleString('it-IT')):'Connessione attiva. Prima sincronizzazione non ancora eseguita.'}</span>`;
    }
  ...
  function ordersEmailHtml(){
    ensureState();
    const drafts=state.emailOrderDraftsV115.filter(d=>d.status!=='Convertita');
