/* Smart Pack · Multiplast — V11.3 accessi separati
...
(()=>{
  ...
  function decorateAuth(){
    ...
    const grid=$('#poiCloudLoginForm .poi-cloud-auth-grid');
    if(grid&&!$('#poi113ForgotAccount')){
      const button=document.createElement('button');button.id='poi113ForgotAccount';button.type='button';button.className='btn';button.textContent='Password account dimenticata?';button.onclick=resetAccountPassword;grid.appendChild(button);
    }
  }
  ...
  function openNomyra(company){
    ...
    $('#poi113Nomyra').classList.add('open');renderAdminTabs();loadEmployees();
  }
  ...
  function updateBuildLabels(){
    ...
    if(foot){const badge=document.createElement('div');badge.id='sp113Build';badge.style.cssText='margin-top:8px;font-size:11px;font-weight:900;opacity:.95';badge.textContent=BUILD+' · Accessi separati';foot.appendChild(badge)}
  }

  function boot(){
    updateBuildLabels();ensureUI();
    [250,700,1500,3000,7200].forEach(delay=>setTimeout(()=>{ensureUI();decorateAuth();decorateCloudMenu();bindPasswordRecovery();updateBuildLabels()},delay));
  }
